...
function initSidebar() {
  ...
  sidebar.innerHTML = `
    <div class="sidebar-logo">
      ...
      <button class="modal-close" onclick="closeSidebar()"
              style="font-size:22px;display:block;" id="sb-close">×</button>
    </div>
    <div class="sidebar-section">Merchant</div>
    ${NAV.map(item => {
      const page   = item.href.split('/').pop();
      const active = current.endsWith(page) || current.endsWith(page.replace('.html',''));
      ...
      ${merchant ? `
        <div style="margin:6px 0;">
          <span class="badge ${merchant.is_active ? 'badge-success' : 'badge-pending'}">
            ${merchant.is_active ? 'Active' : 'Setting up…'}
          </span>
        </div>` : ''}
      <br/>
      <a href="#" onclick="handleLogout()"
         style="font-size:12px;color:var(--text-hint);">Sign out</a>
    </div>`;
  ...
  fetch('/api/health')
    .then(r => r.json())
    .then(d => {
      if (d.config?.supabase_url)
        window.__SUPABASE_URL__ = d.config.supabase_url;
      if (d.config?.supabase_anon_key)
        window.__SUPABASE_ANON_KEY__ = d.config.supabase_anon_key;
    })
    .catch(() => {});
}
